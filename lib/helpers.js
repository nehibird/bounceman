...
// The booking that makes this caller a CUSTOMER rather than a sales lead, or null.
//
// Sarah is a sales agent. Once someone has money down and a date on the calendar their
// calls are support, and support belongs to a human. This is what the call router uses to
// decide, and unlike a direct_route settings row it needs no upkeep and cannot lapse --
// it is derived from the booking, so it switches on when they book and off `graceDays`
// after their event ends, at which point they are a lead again.
//
// graceDays covers the next-morning pickup question.
//
// Returns null on any error. The caller treats that as "not a booked customer" and falls
// through to the normal path, which is the same thing it does when the lookup says no.
function activeBookingForPhone(db, phone, graceDays) {
  const digits = String(phone || '').replace(/\D/g, '').slice(-10);
  if (digits.length !== 10) return null;
  const grace = Number.isFinite(Number(graceDays)) ? Number(graceDays) : 1;
  try {
    return db.prepare(`
      SELECT b.booking_number, b.event_date, b.status, c.first_name || ' ' || c.last_name AS name
      FROM bookings b JOIN customers c ON c.id = b.customer_id
      WHERE b.status NOT IN ('cancelled', 'declined')
        AND date(COALESCE(NULLIF(b.event_end_date, ''), b.event_date)) >= date('now', 'localtime', ?)
        AND replace(replace(replace(replace(replace(replace(COALESCE(c.phone, ''),
            '(', ''), ')', ''), ' ', ''), '-', ''), '+', ''), '.', '') LIKE '%' || ?
      ORDER BY b.event_date LIMIT 1`).get('-' + grace + ' day', digits) || null;
  } catch (e) {
    console.error('[ROUTING] activeBookingForPhone failed:', e.message);
    return null;
  }
}
