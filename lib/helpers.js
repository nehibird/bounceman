...
function priceForBooking(db, eq, { duration, days = 1, wet = false, date = null } = {}) {
  const effectiveDuration = duration === 'multiday' ? 'daily' : duration;
  const base = getPrice(eq, effectiveDuration);

  const extraDays = Math.max(0, (days || 1) - 1);

  let extraRate = 0;
  if (extraDays > 0) {
    const perUnitRate = (eq.price_extra_day !== null && eq.price_extra_day !== undefined)
