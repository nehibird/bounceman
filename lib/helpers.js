...
// These lived only inside /api/sarah/create-and-send-link, which the live assistant no
// longer calls — it calls sendCheckoutLink, which had none of them. Returns null when the
// booking is allowed, otherwise { error, requires_approval? } ready to hand back to the model.
//   date      — ISO YYYY-MM-DD
...
function validateBookingDate(db, date, { duration = 'daily', startTime = '09:00' } = {}) {
  ...
  // Past dates (compared in Central, not UTC)
  if (date < todayCT()) {
  ...
  if (monthIdx < 3 || monthIdx > 10) {
    return { error: `We're closed for the season from December through March — we start back up in April. Want me to help you book a date then?` };
  }
...
function surfaceSurcharge(subtotal, surfaceType, venueType) {
  const s = String(surfaceType || '').toLowerCase();
  const v = String(venueType || '').toLowerCase();
  const hard = /concrete|asphalt|cement|pavement/.test(s) || /indoor/.test(v);
  return hard ? round2(parseFloat(subtotal || 0) * 0.10) : 0;
}
...
function calcPricing(settings, subtotal, deliveryFee, deliveryCity, taxExempt, surfaceFee = 0) {
...
function resolveDate(input) {
  ...
  // Compare CALENDAR DAYS, not instants. `now` carries a time-of-day, so `dt < now` was
  // true for today's own date from 00:00 onward — a caller saying today's date ("July
  // twenty-seventh" on July 27th) got rolled to 2027-07-27. isPast() is date-only.
